...
%Read the soma filter. If not necessary use empyt mask. 
%Mask = zeros(info.Height, info.Width, num_images,'uint8');
...
%% Conventional image as a filter. 
%Use the adaptive thresholding in MATLAB. Change the 'sen' value if
%necessary. 
%Higher value means less stringent filter. 
DYs = false(size(BG));
sen = 0.5;
...
imwrite(uint8(DYs(:,:,1)).*BG(:,:,1),[outpath 'Gconv_filter_' sprintf('%03d',1) '_' char(string(sen)) '.tif']);
...
%0.1416
%% STORM binarize. 
%Also get an image to quickly check the result. 
disp('making CG')
CG = false(size(S2));
...
% end
%% Cluster processing. 
% Get 3D clusters from the binarized STORM image stack. Exclude clusters
% whose depth is not larger than 1. Print the percentage of clusters that
% are filtered out. 
...
disp(rate);
%% Cluster watershedding. 
%Split two clusters if they are too close. When it's is not necessary. Set
%'I3 = imhmin(I2,255);
sizeBG = size(BG);
clear CG CCG
clear statsG2a
...
% Delete clusters that are not larger than 4 pixels. They'll cause erros in
% further processing. 
areacutoff = 4 ;
...
rcentG = centG; 
%% Heat map selection. 
% Change the cutoffg value to get better-looking heatmap. 
i=4;  % i = range(8)
...
if IsManual == 1
    ...
    %synapse_regiong = [0.6437,6.4941;0.7193,5.0678;0.9444,4.6677;0.9787,8.0596;0.8936,10.2860;0.6863,9.0858];
    synapse_regiong = [0.6465,8.6759;0.9002,3.5642;1.0040,3.7444;1.0008,10.1847;0.7805,9.9595];
end
